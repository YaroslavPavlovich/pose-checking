...
def parse_arguments():
    ...
    parser.add_argument('--results-model', type=str, default='pose_checking.pt', help='results training weights path')
    parser.add_argument('--dataset', type=str, default='pose', help='path for dataset')
    parser.add_argument('--labels', type=str, default='normal.json', help='folder of labels with points of pose')
...
def main(opt):
    ...
    if not os.path.isfile(name_dir + '/' + opt.labels):
        ...
        not_normal_points = make_points(path=name_dir + '/' + opt.normal_frames + '/', model=yolo_model)
